...
class Demo:
    ...
    def Refresh(self, time_to_expiry=None):
        ...
        if time_to_expiry is not None:
            request.add_attr(turnattributes.Lifetime, time_to_expiry)
            resp = self.Request(request, time_to_expiry is not None and time_to_expiry != 0)
        return request, resp
        ...
        while True:
            ...
            if ioe[0] or tte == 115:
                ...
                if tte < 110:
                    ...
                    self.lifetime = resp.get_attr(turn.ATTR_LIFETIME)
                    print('[rund] refresh:', resp.format())
                    print('[rund] life time:', self.lifetime.time_to_expiry)
                    tte = self.lifetime.time_to_expiry // 10
        self.running = False
...
def main():
    ...
    remoteport = int(config['turnport'])
